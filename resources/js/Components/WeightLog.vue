<template>
    <li class="py-1.5 flex justify-between border-b border-gray-400 mb-1">
        <p class="w-1/5 text-center">{{ recordDate !== null && recordDate.toLocaleDateString('FR') }}</p>
        <p class="w-1/5 text-center">{{ log.weight }}</p>
        <p class="w-1/5 text-center">{{ log.bmi }}</p>
        <p class="w-1/5 text-center">{{ log.bfp }}</p>
